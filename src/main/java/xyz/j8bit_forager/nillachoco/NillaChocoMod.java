...
public class NillaChocoMod {
    ...
    public static class ClientModEvents {
        ...
        public static void onClientSetup(FMLClientSetupEvent event) {
            ...
            // arm pose
            event.enqueueWork(() -> {
                FLOATIE_ON = HumanoidModel.ArmPose.create("floatie_on", false, (model, entity, arm) -> {
                    if (arm.getId() == 0) {
        ...
        public static void itemColorsRegistryHandler(RegisterColorHandlersEvent.Item event) {
            event.register((stack, color) -> color > 0 ? -1 : ((ApronItem) stack.getItem()).getColor(stack),
                    ModItems.APRON.get());
    ...
    public static class ModEvents {
        ...
        public static void onEntityGetHit(LivingHurtEvent event){
            ...
            if (target.getItemBySlot(EquipmentSlot.LEGS).getItem() instanceof DonutFloatieItem) {
                if (event.getSource().getEntity() instanceof LivingEntity attacker) {
                    Vec3 bounceVec = attacker.position().subtract(attacker.position());
                    bounceVec = bounceVec.normalize().scale(0.25);
                    bounceVec = new Vec3(bounceVec.x, Math.max(0.25, bounceVec.y), bounceVec.z);
                    attacker.setDeltaMovement(bounceVec);
                }
            }
        }
